...
try {
    ...
    if (!$provider || $provider['provider_id'] != $booking['provider_id']) {
        echo json_encode(['success' => false, 'message' => 'You do not have permission to accept this booking']);
        exit;
    }

    // Update booking status to confirmed
    $booking_controller = new booking_controller();
    $result = $booking_controller->update_booking_status_ctr($booking_id, 'confirmed', $response_note);

    echo json_encode($result);
} catch (Exception $e) {
    error_log('Error accepting booking: ' . $e->getMessage());
    echo json_encode(['success' => false, 'message' => 'Error accepting booking: ' . $e->getMessage()]);
}
exit;
?>
